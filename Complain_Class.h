...
private:
    ...
    AVLTree<int, int> userIndex; // maps userID -> index in heap
    int nextComplaintID;
...
public:
    ...
    void increasePriority(int userID) {
        ...
        heapifyUp(index); // move it upwards since it became platinum
    }
    ...
    vector<Complaint> getComplaintsByUser(int userID) const {
        ...
        if (indexPtr) {
            int index = *indexPtr;
            if (index >= 0 && index < heap.size() && heap[index].getUserID() == userID) {
