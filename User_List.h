...
private:
    ...
    AVLTree<int, DLLNode<User>*> userIDIndex;
    AVLTree<string, DLLNode<User>*> userNameIndex;
    AVLTree<string, DLLNode<User>*> emailIndex;
    GroupAVLTree<string> countryIndex;
    GroupAVLTree<string> typeIndex;
...
public:
    ...
    vector<User> getUsersByCountry(const string& country) {
        ...
        if (node) {
            for (DLLNode<User>* userNode : node->users) {
